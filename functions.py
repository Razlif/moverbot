...
conversation_starter_template = environment.from_string(
    prompts.conversation_starter_raw_text)

conversation_starter = conversation_starter_template.render(
...
price_reply_template_ld = environment.from_string(prompts.price_reply_raw_text_ld)
...
# get move type
def get_move_type(pickup_state, delivery_state):
    if pickup_state == delivery_state:
        type = "local move"
    else:
        type = "long distance move"
    return type
    ...
    for i in data_results:
        try:
            ...
            if item_cf > 0:
                ...
                item_list_display += i['name'] + " x " + \
                    str(item_quantity) + " | " + str(total_item_cf) + " CF <br>"
            ...
            problem_items.append(i['name'])
    if problem_items != []:
        item_list_display += problem_items_template.render(
            problem_items=problem_items)
    return item_list_display, all_items_cf


# calculate price for long distance move based on cubic ft and states
def calculate_price_long_distance(delivery_state, cubic_ft):
    price_list = settings.price_list
    cf_reference = ""
    if int(cubic_ft) < settings.minimum_cf_per_job:
        cubic_ft = cf_reference = str(settings.minimum_cf_per_job)
    else:
        marker = [*map(int, str(cubic_ft))]
        marker[-1] = marker[-2] = "0"
        for digit in marker:
            cf_reference += str(digit)
    rates = price_list.loc[price_list.cubic_ft_state ==
                           delivery_state, cf_reference]
    discount = 1 - (settings.discount / 100)
    gas_charge = 1 + (settings.gas_charge / 100)
    for rate in rates:
        price = int(float(rate) * float(cubic_ft) * gas_charge * discount)
    price_reply = price_reply_template_ld.render(
        CUBIC_FT=cubic_ft, DELIVERY_STATE=delivery_state, PRICE=price)
    return price_reply

# calculate price for local move based on cubic ft and mileage fee


def calculate_price_local(pickup_zip, delivery_zip, cubic_ft):
    discount = 1 - (settings.discount / 100)
    ...
    delivery_to_dispatch = get_distance(
        delivery_zip, settings.dispatch_zipcode)
    mileage_fee = (int(disptch_to_pickup) + int(pickup_to_delivery) +
                   int(delivery_to_dispatch)) * int(settings.mileage_fee)
    ...
    if add_hours < 1:
        ...
        price = (int(working_hours * int(settings.price_per_hour)) + int(mileage_fee)) * discount
        price_reply = price_reply_template .render(
            CUBIC_FT=cubic_ft,
            WORKING_HOURS=working_hours,
            WORKING_HOURS_PLUS=working_hours_plus,
            PRICE_PER_HOUR=settings.price_per_hour,
            PRICE=price)
    else:
        ...
        price_reply = price_reply_template .render(
            ...
            WORKING_HOURS_PLUS=int(working_hours_plus),
            PRICE_PER_HOUR=settings.price_per_hour,
            PRICE=int(price))
    return price_reply
...
def check_message_for_errors(stage, reply_text, last_message, conversation_history, user_text):
    if reply_text in last_message or reply_text == user_text or 'CLIENT:' in reply_text or len(
            reply_text) > 150:
...
def main_handler(
        ...
        client_email):
    if stage == 1:
        pickup_state, pickup_zipcode, stage = check_user_text_for_location(
            user_text, stage)
        if pickup_state == "None":
            general_task = general_task_template.render(
                ASK_FOR="pickup zip code")
            prompt = conversation_starter + settings.questions + \
                general_task + user_text + " BOT:"
            ai_response = open_ai_api(prompt)
            reply_text = check_message_for_errors(
                stage, ai_response, last_message, conversation_history, user_text)
        else:
            if pickup_state in settings.allowed_pickup_locations or pickup_zipcode in settings.allowed_pickup_locations:
                reply_text = "Great thanks" + prompts.ask_for_delivery_zipcode
            else:
                reply_text = prompts.out_of_service_area
                stage = 1
    elif stage == 2:
        delivery_state, delivery_zipcode, stage = check_user_text_for_location(
            user_text, stage)
        if delivery_state == "None":
            general_task = general_task_template.render(
                ASK_FOR="delivery zip code")
            prompt = conversation_starter + settings.questions + \
                general_task + user_text + " BOT:"
            ai_response = open_ai_api(prompt)
            reply_text = check_message_for_errors(
            ...
            if delivery_state in settings.allowed_delivery_locations or delivery_zipcode in settings.allowed_delivery_locations:
                reply_text = "Great thanks" + prompts.ask_for_item_list
            else:
                reply_text = prompts.out_of_service_area
                stage = 2
    elif stage == 3:
        stage, client_inventory, reply_text = collect_items(
            user_text, client_inventory, stage)
        if stage == 4:
            prompt = settings.inventory_list + " client item list: " + \
                str(client_inventory) + " " + prompts.inventory_task
            ai_response = open_ai_api(prompt)
            item_list_display, all_items_cf = calc_cf(ai_response)
            move_type = get_move_type(pickup_state, delivery_state)
            if move_type == "local move":
                price_reply = calculate_price_local(
                    pickup_zipcode, delivery_zipcode, int(all_items_cf))
                reply_text = item_list_display + price_reply + prompts.ask_for_email
                stage += 1
            else:
                price_reply = calculate_price_long_distance(
                    delivery_state, int(all_items_cf))
                reply_text = item_list_display + price_reply + prompts.ask_for_email
                stage += 1
    elif stage == 5:
        ...
        general_task = general_task_template.render(
            ASK_FOR="any additional questions")
        prompt = conversation_starter + settings.questions + \
            general_task + user_text + " BOT:"
        ai_response = open_ai_api(prompt)
        reply_text = check_message_for_errors(
            stage, ai_response, last_message, conversation_history, user_text)
    reply_text += "<br>"
    return reply_text, stage, pickup_zipcode, delivery_zipcode, pickup_state, delivery_state, mileage_fee, client_inventory, move_type, client_name, client_phone, client_email
